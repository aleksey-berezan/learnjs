...
function createComment(url, request) {
  ...
  if (!comment.body || !comment.articleId || !comment.username) {
    return { status: 400 };
  }

  if (!database.users[comment.username] || !database.articles[comment.articleId]) {
    return { status: 400 };
  }

  const id = database.nextCommentId;
  const newComment = {
    id: id,
    body: comment.body,
  ...
  database.users[comment.username].commentIds.push(id);
  database.articles[comment.articleId].commentIds.push(id);
...
function deleteComment(url) {
  ...
  userCommentIds = database.users[savedComment.username].commentIds;
  userCommentIds.splice(userCommentIds.indexOf(id), 1);

  articleCommentIds = database.articles[savedComment.articleId].commentIds;
...
function upvoteComment(url, request) {
  ...
  const upvotedComment = upvote(savedComment, username);
  return { status: 200, body: { comment: upvotedComment } };
}
...
function downvoteComment(url, request) {
  ...
  const downvotedComment = downvote(savedComment, username);
  return { status: 200, body: { comment: downvotedComment } };
}
